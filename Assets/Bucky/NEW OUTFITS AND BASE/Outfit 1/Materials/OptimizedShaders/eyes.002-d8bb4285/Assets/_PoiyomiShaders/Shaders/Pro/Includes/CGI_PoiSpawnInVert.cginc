    ...
    void applySpawnInVert(inout float4 worldPos, inout float4 localPos, float2 uv)
    {
        ...
        {
            float noise = 0;
            float gradient = calculateGradientValueVert(float4(0,2,0,1), float4(0,-2,0,1), localPos);
        ...
        //float noise = tex2Dlod(_SpawnInNoise, float4(TRANSFORM_TEX(uv, _SpawnInNoise))).r * float(0) * float(0.35);
    }
