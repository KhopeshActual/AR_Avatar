    ...
    float3 calculateEmissionNew(in float4 baseColor, inout float4 finalColor)
    {
        ...
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(1)) * lerp(1, baseColor, float(0)).rgb * float4(1,1,1,1).rgb;
        }
