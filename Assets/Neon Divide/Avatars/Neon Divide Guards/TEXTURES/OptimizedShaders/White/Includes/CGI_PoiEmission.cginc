...
float3 calculateEmissionNew(in float3 baseColor, inout float4 finalColor)
{
    ...
    float glowInTheDarkMultiplier0 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0));
        ...
        if (!float(0))
        {
            emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiMesh.uv[float(2)], float4(0,0,0,0)).rgb * lerp(1, baseColor, float(0)).rgb * float4(2,2,2,1).rgb;
        }
        else
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * _EmissionMap_ST.xy) + _Time.x * float(5)).rgb * lerp(1, baseColor, float(0)).rgb * float4(2,2,2,1).rgb;
        }
    #else
    ...
    if (float(0))
    {
        ...
        {
            #if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
                emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(2)], _EmissionScrollingCurve) + (dot(pos, float4(0,-10,0,0).xyz) * float(20)) + _Time.x * float(10)).r;
            #endif
        }
        else
        {
    ...
    emissionColor0 = hueShift(emissionColor0, frac(float(0) + float(0) * _Time.x) * float(0));
        ...
        if (float(4) != 4)
        {
            emissionMask0 *= blackLightMask[float(4)];
        }
    #endif
    ...
    float3 emissionColor1 = 0;
        ...
        emissionColor1 = hueShift(emissionColor1, frac(float(0) + float(0) * _Time.x) * float(0));
            ...
            if (float(4) != 4)
            {
                emissionMask1 *= blackLightMask[float(4)];
            }
        #endif
